...
func (h *BansHandler) GetActiveBans(c *gin.Context) {
	...
	result, err := h.banSvc.GetActiveBans(page, perPage, search, sortBy, sortDir, country)
	if err != nil {
		response.InternalError(c, "Failed to fetch active bans")
		return
	}
	response.OK(c, result)
}

// GetBanHistory returns paginated ban history.
func (h *BansHandler) GetBanHistory(c *gin.Context) {
	...
	result, err := h.banSvc.GetBanHistory(page, perPage, search, dateFrom, dateTo)
	if err != nil {
		response.InternalError(c, "Failed to fetch ban history")
		return
	}
	response.OK(c, result)
}
...
func (h *BansHandler) UnbanIP(c *gin.Context) {
	...
	h.auditSvc.Log(userID.(int64), username.(string), "unban_ip", ban.IPAddress, map[string]interface{}{
		"ban_id": id,
		"jail":   ban.Jail,
	}, c.ClientIP())
...
func (h *BansHandler) BulkUnban(c *gin.Context) {
	...
	userID, _ := c.Get("user_id")
	username, _ := c.Get("username")
	h.auditSvc.Log(userID.(int64), username.(string), "bulk_unban", "", map[string]interface{}{
		"count": affected,
		"ids":   req.IDs,
	}, c.ClientIP())
